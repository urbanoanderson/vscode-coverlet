import { stripReport } from '../stripReport';
...
describe('Strip Report', () => {
  ...
  it('Not a JSON should return null', () => {
    const result = stripReport("invalid report");
    expect(result).to.equal(null);
  });

  it('JSON without \'lines\' should parse', () => {
    const result = stripReport(JSON.stringify({"App":{"File":{"Class":{"Method":{"Branches":[]}}}}}));
    expect(JSON.stringify(result)).to.equal(JSON.stringify({"App":{}}));
  });

  it('JSON without \'branches\' should parse', () => {
    const result = stripReport(JSON.stringify({"App":{"File":{"Class":{"Method":{"Lines":{"6":1,"7":1,"8":1}}}}}}));
    expect(JSON.stringify(result)).to.equal(JSON.stringify({"App":{}}));
  });

  it('JSON branch does not contain Hits should return null', () => {
    const result = stripReport(JSON.stringify({"App":{"File":{"Class":{"Method":{"Branches":[{"Line":17,"Offset":10,"EndOffset":16,"Path":1,"Ordinal":1}]}}}}}));
    expect(result).to.equal(null);
  });
  ...
  it('Happy path should strip JSON', () => {
    ...
    const result = stripReport(input);
